...
The compiler represents four closed activated effects that make one target
creature unable to attack, block, both, or be blocked until end of turn. Their
initial implementation stored the resolved result as a layer-6 added ability
fragment. That made `remove_all_abilities` erase a rule already created by a
resolving ability, even though removing the target's abilities cannot end that
independent effect.
...
- Generalize source-relative, conditional, dynamic, or non-turn durations in
  this correction. Rejected because those forms require additional binding and
  characteristic-dependency semantics.

## Consequences

- The existing activated grammar and CardProgram operation remain compatible;
  only the runtime result changes ownership.
